...
public class UserRegistrationWKeyboardReaction extends AbstractPrivateChatReaction {
    ...
    public void react(final Update update, final BiConsumer<Long, Object> sender, final String commandLine) {
        if (update.hasMessage()) {
            final Message message = update.getMessage();
            final boolean isKnown = registrationService.isKnownUser(message.getFrom().getId(), message.getChatId());
            if (!isKnown) {
                sender.accept(message.getChatId(), new SendMessage(String.valueOf(message.getChatId()), """
                        Не видел Вас в чате дома. Вы там что-нибудь когда-нибудь писали? Напишите в чате дома Ы!"""));
                return;
            }

            final boolean isFromAddress = registrationService.isFromAddress(message.getFrom().getId());
            if (!isFromAddress) {
                sender.accept(message.getChatId(), new SendMessage(String.valueOf(message.getChatId()), """
                        Вы сказали, что живете не в этом доме. Функции бота Вам недоступны"""));
                return;
            }

            final boolean hasText = message.hasText();
            if (hasText) {
                processMessageText(message.getText(), message.getChatId(), message.getFrom().getId(), sender);
            }
            sendNextQuestion(message.getChatId(), message.getFrom().getId(), sender);
        } else if (update.hasCallbackQuery()) {
            final CallbackQuery callbackQuery = update.getCallbackQuery();
            final String data = callbackQuery.getData();
            final CallbackQueryParsed parsed = callbackQueryDataHelper.parseCallbackQueryData(data);

            final User user = userRepository.getUserByTgUserId(parsed.tgUserId());
            final RegistrationQuestion question = registrationService.getNextRegistrationQuestion(user);
            saveAnswer(parsed.answer(), user, question);
            sendNextQuestion(callbackQuery.getMessage().getChatId(), user.getTgUserId(), sender);
        }
    }
