...
@Component
public class GetNeighborNearCommand extends AbstractPrivateChatReaction {
    private final UserRepository userRepository;

    @Autowired
    public GetNeighborNearCommand(final UserRepository userRepository) {
    ...
    public void react(final Update update, final BiConsumer<Long, Object> sender, final String commandLine) {
        ...
        String text = users.isEmpty()
                ? "Извините, я пока ничего не знаю о Ваших непосредственных соседях по этажу"
                : "Извините, я пока не умею определять Ваших непосредственных соседей по этажу, но вот все, кого я знаю с Вашего этажа - " + users; // TODO: надо научиться считать
